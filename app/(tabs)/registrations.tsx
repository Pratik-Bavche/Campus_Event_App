...
export default function RegistrationsScreen() {
    const { myRegistrations, fetchRegistrations, isRegistrationsLoading, cancelRegistration } = useDataStore();
    ...
    const filteredRegistrations = myRegistrations.filter(reg => {
        ...
        if (selectedTab === 'Completed') return new Date(reg.event.date) <= new Date() && reg.status !== 'cancelled';
        ...
        return (
            <Card style={[styles.regCard, isCancelled && { opacity: 0.7 }]}>
                <View style={styles.eventRow}>
                    <Image source={{ uri: item.event.poster }} style={styles.eventImage} />
                    <View style={styles.eventInfo}>
                        <View style={styles.titleRow}>
                            <Text style={[styles.eventName, { color: colors.text }]} numberOfLines={2}>{item.event.name}</Text>
                            <View style={[
                                styles.statusBadge,
                                { backgroundColor: isCancelled ? colors.error + '15' : isCompleted ? colors.success + '15' : colors.primary + '15' }
                            ]}>
                                <Text style={[
                                    styles.statusText,
                                    { color: isCancelled ? colors.error : isCompleted ? colors.success : colors.primary }
                                ]}>
                        ...
                        <View style={styles.clubRow}>
                            <Text style={[styles.clubName, { color: colors.textMuted }]}>{item.event.club}</Text>
                        </View>
                        ...
                        <View style={styles.detailsRow}>
                            <View style={styles.detailItem}>
                                ...
                                <Text style={[styles.detailText, { color: colors.textMuted }]}>
                                    {new Date(item.event.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                                </Text>
                            </View>
                            <View style={styles.detailItem}>
                                <Clock size={14} color={colors.textMuted} />
                                <Text style={[styles.detailText, { color: colors.textMuted }]}>
                                    {new Date(item.event.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}
                                </Text>
                            </View>
                        </View>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            <Text style={{ color: colors.textMuted }}>No events found in this category.</Text>
                        </View>
                    }
                />
            )}
